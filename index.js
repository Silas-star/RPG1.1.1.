...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oi @${num.split('@')[0]}\ bem vindo ao grupo *${mdata.subject}* Mande !menu pra saber de coisas como a ficha,nerfs,histórias e sistemas e mais opções lembre-se de se registrar no !registrar {seu nomer}|{sua idade} para usar o menu`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ Calma ae ta processando,se não vier mand denovo',
				success: '️❬ ✔ ❭ 𝘀𝘂𝗰𝗰𝗲𝘀𝘀o 🖤',
				error: {
					stick: 'Calma ai😎🤝🏻',
					Iv: '⚠️Erro⚠️'
			...
			const ownerNumber = ["5548998006626@s.whatsapp.net"] 
          ...
          case 'ping':
            ...
            break	
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(mess.only.ownerB)
					prefix = args[0]
					reply(`Prefix mudado com sucesso para : ${prefix}📌`)
					break 	
				case 'meme': 
